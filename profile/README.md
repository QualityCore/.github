...
#### 🔥 끓임공정
끓임공정은 여과된 맥즙에 홉을 투입하여 고온에서 끓이는 단계입니다.

해당 공정은 끓임설정정 **온도가 100°C에 도달한 시점** 부터 자동으로 시작되며, 초기 워트량은 **여과공정에서 최종 회수 워트량**을 기준으로 자동 설정됩니다.
홉 투입 정보는 작업지시서에 등록된 자재 정보를 기반으로 자동 불러오며, 첫 번째 홉과 두 번째 홉의 투입량을 입력할 수 있습니다.

끓임 종료 시에는 **끓임 손실량(초기 워트량의 5%)** 이 자동 계산되어 표시되며,
최종적으로 끓임 후 워트량 = 초기 워트량 - 끓임 손실량 으로 산출되어 실시간 업데이트됩니다.

이를 통해 작업자는 **홉 처리 시점과 수율 변화** 를 직관적으로 파악하고, 다음 공정으로 원활하게 연계할 수 있습니다.
...
온도가 설정값에 도달하면 모달창이 표시되고, 사용자가 확인하면 냉각공정이 시작됩니다. 공정이 완료되면 자동으로 다음 단계로 넘어갑니다.
